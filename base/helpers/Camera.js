...
export class Camera {
    ...
    handleKeys(elapsed) {
        ...
        if (this.currentlyPressedKeys[83] && this.cameraInFront()) {	//S
            if (this.camPosY <= 2) {
                this.camPosY = 2
            } else {
        ...
        if (this.currentlyPressedKeys[83] && this.cameraInBack()) {	//S
            if (this.camPosY <= 2) {
                this.camPosY = 2
            } else {
                rotateVector(-2, camPosVec, 1, 0, 0);  //Roterer camPosVec 2 grader om x-aksen.
        ...
        if (this.currentlyPressedKeys[87] && this.cameraInFront()) {	//W
            if (this.camPosY >= this.camPosZ * 1.5) {
                this.camPosY = this.camPosZ * 1.5
            } else {
                rotateVector(-2, camPosVec, 1, 0, 0);  //Roterer camPosVec 2 grader om x-aksen.
            }
        }
        if (this.currentlyPressedKeys[87] && this.cameraInBack()) {	//W
            if (this.camPosY >= this.camPosZ * -1.5) {
                this.camPosY = this.camPosZ * -1.5
            } else {
                rotateVector(2, camPosVec, 1, 0, 0);  //Roterer camPosVec 2 grader om x-aksen.
